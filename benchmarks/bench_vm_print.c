...
enum {
  VM_PRINT_SOURCE_MAX = 4096
};

static volatile const char *g_vm_print_name_sink = NULL;
static volatile uint64_t g_vm_print_bytes_sink = 0U;
...
int main(int argc, char **argv) {
  char source[VM_PRINT_SOURCE_MAX];
  ...
  count_slot = find_global_slot(&program, "copy_count");
  name_slot = find_global_slot(&program, "copy_name");
  if (count_slot < 0 || name_slot < 0) {
    graphion_runtime_program_dispose(&program);
    fprintf(stderr, "required globals not found in prepared workload\n");
    return 8;
  }
  ...
  for (i = 0; i < iterations; ++i) {
    ...
    g_vm_print_name_sink = globals[(size_t)name_slot].as.string_value;
    g_vm_print_bytes_sink = count_sink.bytes_written;
    checksum += (uint64_t)globals[(size_t)count_slot].as.int_value;
  }
  end = now_seconds();
  ...
  printf("{\"benchmark\":\"vm_print_dispatch\",\"iterations\":%ld,\"instructions_per_iteration\":%zu,"
         "\"print_ops_per_iteration\":5,\"seconds\":%.6f,\"mips\":%.3f,"
