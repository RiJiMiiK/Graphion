...
static int execute_condition_program(const graphion_runtime_program *program,
  ...
  if (rc != GVM_OK) {
    ...
    if (rc == GVM_ERR_LOG10_DOMAIN) {
      return fail(diagnostic, line, 1U, "log10 requires strictly positive input", GINT_ERR_RUN);
    }
...
int graphion_execute_prepared_program_with_sink(const graphion_runtime_program *program,
  ...
  if (rc != GVM_OK) {
    ...
    if (rc == GVM_ERR_LOG10_DOMAIN) {
      return fail(diagnostic, 1U, 1U, "log10 requires strictly positive input", GINT_ERR_RUN);
    }
    if (rc == GVM_ERR_FACTORIAL_DOMAIN) {
