...
int test_gion_graph_declaration_column_diagnostics(void) {
  static const struct {
    ...
    unsigned int alternate_column;
    const char *message;
  } cases[] = {
      {"graph;\n", GINT_ERR_PARSE, 1U, 6U, 0U, "expected graph name"},
      {"graph G\n", GINT_ERR_PARSE, 1U, 8U, 0U, "expected ';' or ':' after graph declaration"},
      {"graph G; extra\n", GINT_ERR_PARSE, 1U, 10U, 0U, "unexpected trailing tokens after graph declaration"},
      {"graph G:\n", GINT_ERR_PARSE, 1U, 9U, 0U, "expected indented graph node block"},
      {"graph G:\n    ?\n", GINT_ERR_PARSE, 2U, 1U, 0U, "expected graph node name or id"},
      {"graph G:\n    \"alpha\" extra\n", GINT_ERR_PARSE, 2U, 9U, 0U, "unexpected trailing tokens after graph node"},
      {"graph G:\n    \"alpha\" {\"a\": missing,}\n",
       GINT_ERR_PARSE,
       2U,
       23U,
      ...
      {"graph G:\n    1 - 2 {\"weight\": missing,}\n",
       ...
       0U,
       "trailing comma is not allowed in dict literal"},
      {"graph G:\n    1 - 2 (missing\n", GINT_ERR_PARSE, 2U, 7U, 0U, "expected ')' after expression"},
      {"graph G:\n    1 -\n", GINT_ERR_PARSE, 2U, 4U, 0U, "expected graph node name or id"},
      {"hypergraph;\n", GINT_ERR_PARSE, 1U, 11U, 0U, "expected hypergraph name"},
      {"hypergraph H\n", GINT_ERR_PARSE, 1U, 13U, 0U, "expected ';' after hypergraph declaration"},
      {"hypergraph H; extra\n",
       GINT_ERR_PARSE,
       1U,
       15U,
      ...
      {"hypergraph H:\n", GINT_ERR_PARSE, 1U, 14U, 0U, "expected indented hypergraph vertex block"},
      {"hypergraph H:\n    ?\n", GINT_ERR_PARSE, 2U, 1U, 0U, "expected graph node name or id"},
      {"hypergraph H:\n    \"Alice\" extra\n",
       GINT_ERR_PARSE,
       2U,
      ...
      {"hypergraph H:\n    [1, 2\n", GINT_ERR_PARSE, 2U, 6U, 0U, "expected ']' after hyperedge vertex list"},
      {"hypergraph H:\n    []\n", GINT_ERR_RUN, 2U, 1U, 0U, "hyperedge must contain at least one vertex"},
      {"hypergraph H:\n    defaults other {}\n",
       GINT_ERR_PARSE,
       2U,
       10U,
  ...
  for (i = 0U; i < sizeof(cases) / sizeof(cases[0]); ++i) {
    ...
    if (diagnostic.line != cases[i].expected_line ||
        (diagnostic.column != cases[i].expected_column &&
         (cases[i].alternate_column == 0U || diagnostic.column != cases[i].alternate_column))) {
      fprintf(stderr,
              "gion_graph_declaration_column_diagnostics case %lu expected %u:%u got %u:%u (%s)\n",
              (unsigned long)i,
              cases[i].expected_line,
              cases[i].expected_column,
              diagnostic.line,
              diagnostic.column,
              diagnostic.message != NULL ? diagnostic.message : "<no diagnostic>");
      return (int)(200 + i);
    }
    if (diagnostic.message == NULL || strcmp(diagnostic.message, cases[i].message) != 0) {
      return (int)(300 + i);
