...
- `02_arithmetic.gion`
  - arithmetic operators, precedence, negative operands, and `abs(...)`
- `03_compound_assignments.gion`
  - `+=`, `-=`, `*=`, `/=`, `//=`, `%=`, `**=`
- `04_conditionals.gion`
  - `if / elif / else`, nested conditionals, grouped multiline conditions, single-line and multiline ternary expressions, equality/ordering conditions (`==`, `!=`, `<`, `<=`, `>`, `>=`), `and` / `nand` / `or` / `nor` / `not`, and accepted `0` / `1` conditions
- `05_comments.gion`
