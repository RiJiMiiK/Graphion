...
- `02_arithmetic.gion`
  - arithmetic operators, precedence, negative operands, unary minus, postfix factorial `!`, the `pi` and `e` constants, numeric builtins like `abs(...)`, `min(...)`, `max(...)`, `clamp(...)`, `sqrt(...)`, `exp(...)`, `ln(...)`, `log(...)`, and the string builtin `len(...)`
