...
- the `abs()`, `min()`, `max()`, `clamp()`, `sqrt()`, `cbrt()`, `sin()`, `sinh()`, `cos()`, `cosh()`, `tan()`, `tanh()`, `asin()`, `acos()`, `atan()`, `atan2()`, `hypot()`, `exp()`, `ln()`, `log()`, `log10()`, `log2()`, `floor()`, `ceil()`, `round()`, `trunc()`, `sign()`, and `len()` builtins, plus postfix factorial `!` and numeric constants like `pi` and `e`
