...
- the `abs()`, `min()`, `max()`, `clamp()`, `sqrt()`, `cbrt()`, `exp()`, `ln()`, `log()`, `log10()`, `log2()`, `floor()`, `ceil()`, `round()`, `trunc()`, `sign()`, and `len()` builtins, plus postfix factorial `!` and numeric constants like `pi` and `e`
